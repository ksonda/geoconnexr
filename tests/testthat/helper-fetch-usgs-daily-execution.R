usgs_daily_test_clone <- function(x) unserialize(serialize(x, NULL))

usgs_daily_test_error <- function(expr) tryCatch(expr, error = identity)

usgs_daily_test_fixture_path <- function(
    name = "daily-single-page.geojson") {
  testthat::test_path("fixtures", "fetch", "usgs", name)
}

usgs_daily_test_body <- function(name = "daily-single-page.geojson") {
  path <- usgs_daily_test_fixture_path(name)
  readBin(path, what = "raw", n = file.info(path)$size)
}

usgs_daily_test_body_replace <- function(from, to) {
  text <- rawToChar(usgs_daily_test_body())
  stopifnot(grepl(from, text, fixed = TRUE))
  charToRaw(sub(from, to, text, fixed = TRUE))
}

usgs_daily_test_url <- function(extra = "") {
  paste0(
    "https://api.waterdata.usgs.gov/ogcapi/v0/collections/daily/items?",
    "monitoring_location_id=USGS-01491000&parameter_code=00060&",
    "statistic_id=00003", extra
  )
}

usgs_daily_test_catalog <- function(url = usgs_daily_test_url()) {
  ...
  catalog$datasets$handler_id[[position]] <- "usgs_waterdata_daily"
...
usgs_daily_test_m7d_plan <- function(
    url = usgs_daily_test_url(), max_response_bytes = 20000,
    max_rows = 10000L, max_columns = 100L) {
  intent_set <- csv_request_plan_test_intent_set(
    catalog = usgs_daily_test_catalog(url),
...
usgs_daily_test_distribution_id <- function(plan) {
  position <- which(
    plan$coverage$handler_id == "usgs_waterdata_daily"
  )
  plan$coverage$distribution_id[[position]]
}

usgs_daily_test_request_plan <- function(
    plan = usgs_daily_test_m7d_plan(), max_fields = 1000L) {
  gx_usgs_daily_request_plan_impl(
    plan,
    distribution_id = usgs_daily_test_distribution_id(plan),
    max_fields = max_fields
  )
}

usgs_daily_test_scope <- function(label = "execution") {
  gx_contract_hash(
    list("fixture", label),
    namespace = "geoconnexr.usgs-daily-test.v1",
    contract_version = "0.1.0"
  )
}

usgs_daily_test_resolver <- function(
...
usgs_daily_test_performer <- function(
    body = usgs_daily_test_body(),
    status = 200L,
...
usgs_daily_test_options <- function(performer) {
  ...
  withr::local_options(
    list(
      ...
      geoconnexr.throttle_sleep = function(seconds) {
        stop("A zero-interval USGS daily execution attempted to sleep.", call. = FALSE)
      }
    ),
...
usgs_daily_test_execute <- function(
    request_plan = usgs_daily_test_request_plan(),
    performer = usgs_daily_test_performer(),
    symbol_resolver = usgs_daily_test_resolver(),
    scope = usgs_daily_test_scope()) {
  usgs_daily_test_options(performer)
