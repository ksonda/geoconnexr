...
The internal M7h checkpoint can preview direct-CSV admission without
network activity or execute bounded CSV requests sequentially with one
exact status per distribution. M7i now adds the first complete non-CSV
handler slice: one reserved OGC API Features items request, a runtime
symbol check immediately before invocation, strict GeoJSON-to-`sf`
normalization, and one charged attempt ledger row. Next-page links are
reported as truncation and are not followed. This remains tested
