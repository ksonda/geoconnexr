...
The internal M7n checkpoint now schedules direct CSV, WQP Result, EDR position,
current USGS continuous and daily data, and OGC API Features in one deterministic global
...
strict fixed GeoJSON table and reports, but never follows, a next page. The
USGS daily path adds an exact statistic code and closed local-date interval,
preserves observation dates as `Date`, and treats a missing match count as
unknown rather than complete. It also performs only one page and never follows
the advertised next link. This
