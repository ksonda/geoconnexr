...
crosswalks, spatial areas of interest, public bounded catalogs, and
provider fetches.
...
Build a catalog from a known Water Quality Portal PID, preview the
bounded plan, and fetch one distribution:
...
- Typed SPARQL template rendering and portable distribution
  classification.
- Bounded public catalog construction from one graph page, explicit
  PIDs, or caller-supplied local JSON-LD profiles.
