.gx_fetch_orchestration_contract_version <- "0.1.0"
...
.gx_fetch_orchestration_metadata_fields <- c(
  ...
  "csv_semantics_validated", "oaf_semantics_validated",
...
.gx_fetch_orchestration_count_fields <- c(
  "distributions", "candidate_requests", "csv_requests", "oaf_requests",
  ...
  "csv_results", "oaf_results", "results"
)
...
gx_fetch_orchestration_input_plan_impl <- function(request_plan) {
  ...
  if (!valid) {
    gx_fetch_orchestration_abort(
      "M7j construction requires one valid M7d all-handler request plan.",
...
gx_fetch_orchestration_flag_impl <- function(x) {
  if (!is.logical(x) || length(x) != 1L || is.na(x) ||
    ...
    gx_fetch_orchestration_abort(
      "The M7j dry-run choice must be one explicit logical value.",
...
gx_fetch_orchestration_byte_limit_impl <- function(x) {
  if (!is.numeric(x) || length(x) != 1L || is.na(x) || !is.finite(x) ||
      ...
      !is.null(attributes(x))) {
    gx_fetch_orchestration_abort(
      "The M7j aggregate response-byte ceiling must be an explicit bounded whole number.",
      "gx_error_fetch_orchestration_policy"
    )
  }
  unname(as.double(x))
...
gx_fetch_orchestration_policy_impl <- function(
  ...
  max_executions <- gx_fetch_orchestration_integer_impl(
    ...
    "The M7j execution ceiling must be an explicit bounded whole number."
  )
  ...
  if (is.null(max_fields) || is.null(oaf_limit) || is.null(timeout) ||
    ...
    gx_fetch_orchestration_abort(
      "M7j parser, OGC page, and timing limits must be explicit bounded values.",
      "gx_error_fetch_orchestration_policy"
    )
  }
  list(
    slice_id = "cross_handler_orchestration_v1",
  ...
  if (length(oaf_positions)) {
    for (position in oaf_positions) {
      ...
      if (!inherits(oaf_plan, "gx_oaf_request_plan")) {
        gx_fetch_orchestration_abort(
          "M7j could not bind an OGC request to its held reservation.",
          "gx_error_fetch_orchestration_input"
        )
      }
      rows[[length(rows) + 1L]] <- tibble::tibble(
...
gx_fetch_orchestration_id_impl <- function(
  ...
  gx_contract_hash(
    ...
    namespace = "geoconnexr.fetch-orchestration.v1",
    contract_version = .gx_fetch_orchestration_contract_version
  )
}
...
gx_fetch_orchestration_child_scope_impl <- function(
  ...
  gx_contract_hash(
    ...
    namespace = "geoconnexr.fetch-orchestration-child.v1",
    contract_version = .gx_fetch_orchestration_contract_version
  )
}
  ...
  statuses <- unname(vapply(seq_len(nrow(coverage)), function(position) {
    ...
    if (!is.na(request_row)) {
      ...
      switch(
        ...
        gx_fetch_orchestration_abort(
          "M7j encountered an unknown M7d coverage status."
        )
      )
    }
  }, character(1)))
...
gx_fetch_orchestration_metadata_impl <- function(
  ...
  oaf_attempted <- any(
    status$handler_id == "ogc_api_features" & status$attempted
  )
  list(
    ...
    runtime_symbols_checked = oaf_attempted,
...
gx_fetch_orchestration_impl <- function(
  ...
  if (is.null(orchestration_scope_id)) {
    gx_fetch_orchestration_abort(
      "M7j requires one explicit opaque orchestration scope identity.",
      "gx_error_fetch_orchestration_policy"
    )
  }
...
gx_fetch_orchestration_validate_policy_impl <- function(policy) {
  ...
  if (is.null(expected) || !identical(policy, expected) || !all(vapply(
    ...
    gx_fetch_orchestration_abort(
      "M7j policy violates its exact bounded contract."
    )
  }
  invisible(policy)
}
...
gx_fetch_orchestration_validate_oaf_result_impl <- function(
  ...
  if (!valid_shape) {
    gx_fetch_orchestration_abort(
      "A compact M7j OGC result violates its exact shape."
    )
  }
  ...
  if (!valid || !identical(
    ...
    gx_fetch_orchestration_abort(
      "A compact M7j OGC result no longer rebinds to its retained bytes."
    )
  }
  invisible(result)
}
...
gx_fetch_orchestration_validate_result_impl <- function(
  ...
  if (inherits(result, "gx_csv_orchestration_result")) {
    ...
    if (!valid) {
      gx_fetch_orchestration_abort(
        "A compact M7j CSV result failed whole-result validation."
      )
    }
    ...
    gx_fetch_orchestration_abort(
      "M7j results must use one supported handler-specific compact contract."
    )
  }
  invisible(result)
}
...
gx_fetch_orchestration_validate_status_impl <- function(
  ...
  if (!valid_types || anyNA(status[c(
    ...
    gx_fetch_orchestration_abort(
      "M7j status violates its exact one-row-per-distribution shape."
    )
  }
  ...
  if (!identical(status[identity_columns], base[identity_columns]) ||
    ...
    gx_fetch_orchestration_abort(
      "M7j status identities or bounded attempt facts are invalid."
    )
  }
  ...
  if (any(status$orchestration_status[nonadmitted] !=
    ...
    gx_fetch_orchestration_abort(
      "M7j non-attempted rows overstate execution or budget facts."
    )
  }
  if (!policy$dry_run) {
    ...
    if (any(!status$orchestration_status[live] %in% terminal) ||
      ...
      gx_fetch_orchestration_abort(
        "Every admitted M7j request must reach one terminal status."
      )
    }
    successful <- which(status$succeeded)
    ...
    if (!identical(
      ...
      gx_fetch_orchestration_abort(
        "M7j terminal rows do not reconcile exactly with compact results."
      )
    }
    if (length(failed)) {
      ...
      if (anyNA(expected_error) ||
        ...
        gx_fetch_orchestration_abort(
          "M7j failure rows do not bind their handler-specific phase."
        )
      }
    }
    if (length(results)) {
      for (index in seq_along(results)) {
        ...
        if (status$handler_id[[row]] != handler ||
          ...
          gx_fetch_orchestration_abort(
            "M7j successful status rows do not bind their compact results."
          )
        }
      }
    }
  ...
  if (any(status$encoded_bytes[planned] >
    ...
    gx_fetch_orchestration_abort(
      "M7j status exceeds an allocated request or aggregate byte budget.",
...
gx_fetch_orchestration_validate_impl <- function(x) {
  ...
  if (!valid_top) {
    gx_fetch_orchestration_abort(
      "M7j orchestration violates its exact top-level contract."
    )
  }
  ...
  if (!identical(x$requests, planning$requests)) {
    gx_fetch_orchestration_abort(
      "M7j candidate requests no longer rebind to the shared M7d plan."
    )
  }
  ...
  if (!valid_orchestration || !identical(
    ...
    gx_fetch_orchestration_abort(
      "M7j orchestration identity no longer binds its plan and policy."
    )
  }
  if (!is.list(x$results) || !is.null(attributes(x$results)) ||
      length(x$results) > x$policy$max_executions) {
    gx_fetch_orchestration_abort(
      "M7j compact results violate their list or cardinality budget."
    )
  }
  if (length(x$results)) {
    for (result in x$results) {
      ...
      if (is.na(request_position)) {
        gx_fetch_orchestration_abort(
          "A compact M7j result has no planned logical request."
        )
      }
      ...
      if (!identical(result$execution$execution_scope_id, expected_scope)) {
        gx_fetch_orchestration_abort(
          "A compact M7j result has a foreign child execution scope."
        )
      }
    }
    ...
    if (anyDuplicated(execution_ids) || anyDuplicated(logical_ids)) {
      gx_fetch_orchestration_abort(
        "M7j compact result identities must be unique."
      )
    }
  }
  ...
  if (!identical(orchestration, expected_orchestration) || !valid_metadata ||
    ...
    gx_fetch_orchestration_abort(
      "M7j orchestration metadata or completion facts are inconsistent."
    )
  }
  ...
  if (!is.finite(owned_text) ||
    ...
    gx_fetch_orchestration_abort(
      "M7j owned text exceeds its aggregate byte budget.",
      "gx_error_fetch_orchestration_budget"
    )
  }
  invisible(x)
...
print.gx_fetch_orchestration <- function(x, ...) {
  ...
  cli::cli_inform(c(
    ...
    paste0(
      "* CSV: ", counts$csv_requests, "; OGC Features: ",
