.gx_usgs_continuous_request_plan_contract_version <- "0.1.0"
.gx_usgs_continuous_execution_contract_version <- "0.1.0"
.gx_usgs_continuous_max_text_bytes <- 64L * 1024L^2
.gx_usgs_continuous_max_json_depth <- 32L
.gx_usgs_continuous_properties <- c(
...
.gx_usgs_continuous_request_plan_fields <- c(
  "contract_version", "request_plan", "policy", "request", "metadata"
)

.gx_usgs_continuous_policy_fields <- c(
...
.gx_usgs_continuous_request_fields <- c(
  "logical_request_id", "reservation_id", "distribution_id", "fetch_order",
  "base_url_redacted", "api_version", "collection_id", "query_type",
  "monitoring_location_id", "parameter_code", "time_start", "time_end",
  "time", "properties", "skip_geometry", "response_format", "language",
  "limit", "source_url_redacted", "canonical_url_redacted",
  "max_physical_attempts", "max_encoded_bytes", "max_decoded_bytes",
  "response_byte_limit", "max_rows", "max_columns", "request_status"
)

.gx_usgs_continuous_plan_metadata_fields <- c(
...
.gx_usgs_continuous_implementation_fields <- c(
...
.gx_usgs_continuous_execution_fact_fields <- c(
...
.gx_usgs_continuous_attempt_columns <- c(
...
.gx_usgs_continuous_execution_metadata_fields <- c(
  "host_specific", "replayable", "execution_ready", "transport_authorized",
...
gx_usgs_continuous_abort <- function(
    message,
    class = "gx_error_usgs_continuous_contract",
    ...,
    call = rlang::caller_env()) {
  gx_abort(
    message,
    class = unique(c(class, "gx_error_usgs_continuous", "gx_error_fetch_plan")),
    ...,
    call = call,
    .redact_trace = TRUE
  )
...
gx_usgs_continuous_scalar_text <- function(x, nonempty = TRUE) {
  is.character(x) && length(x) == 1L && !is.na(x) &&
    (!nonempty || nzchar(x)) && is.null(attributes(x))
}

gx_usgs_continuous_exact_names <- function(x, expected) {
  is.list(x) && identical(names(x), expected) &&
    identical(names(attributes(x)), "names")
}

gx_usgs_continuous_bounded_text_impl <- function(
    x, label, allow_empty = FALSE, maximum = 2048L) {
  valid <- gx_usgs_continuous_scalar_text(x, nonempty = !allow_empty) &&
  ...
  if (!valid) {
    gx_usgs_continuous_abort(
      "The USGS continuous {label} is not one bounded control-safe UTF-8 value.",
...
gx_usgs_continuous_policy_impl <- function(max_fields) {
  max_fields <- tryCatch(
  ...
  if (is.null(max_fields)) {
    gx_usgs_continuous_abort(
      "The USGS continuous parser field limit must be one explicit bounded whole number.",
      "gx_error_usgs_continuous_plan_policy"
    )
  }
  list(
    slice_id = "usgs_continuous_single_page_v1",
    handler_id = "usgs_waterdata_continuous",
    implementation_id = "geoconnexr:dataRetrieval-continuous",
    implementation_package = "dataRetrieval",
    minimum_version = "2.7.22",
    query_symbol = "read_waterdata_continuous",
    normalization_owner = "geoconnexr",
    method = "GET",
    query_type = "continuous",
    ...
    max_json_depth = .gx_usgs_continuous_max_json_depth,
    max_json_members = unname(as.integer(3L * max_fields + 1024L)),
    properties = .gx_usgs_continuous_properties
  )
}

gx_usgs_continuous_source_row_impl <- function(request_plan, distribution_id) {
  valid <- tryCatch({
    gx_csv_request_plan_validate_impl(request_plan)
    TRUE
  }, error = function(cnd) FALSE, warning = function(cnd) FALSE)
  if (!valid || !gx_usgs_continuous_scalar_text(distribution_id) ||
      !gx_catalog_is_sha256(distribution_id)) {
    gx_usgs_continuous_abort(
      "M7m planning requires a valid M7d plan and one distribution identity.",
  ...
  if (!positions_valid) {
    gx_usgs_continuous_abort(
      "The selected USGS continuous distribution has no complete M7d reservation.",
      "gx_error_usgs_continuous_plan_input"
    )
  }
...
gx_usgs_continuous_time_impl <- function(distribution) {
  ...
  if (!valid) {
    gx_usgs_continuous_abort(
      "The USGS continuous distribution requires one finite planned time interval.",
...
gx_usgs_continuous_query_value_impl <- function(query, names, name) {
  position <- match(name, names)
  if (is.na(position)) NULL else query[[position]]
}

gx_usgs_continuous_target_impl <- function(
    source_url, distribution, max_rows, max_fields) {
  if (!gx_usgs_continuous_scalar_text(source_url)) {
    gx_usgs_continuous_abort(
      "The USGS continuous source URL is invalid.",
      "gx_error_usgs_continuous_plan_url"
    )
  }
  canonical <- tryCatch(
  ...
  if (is.null(canonical)) {
    gx_usgs_continuous_abort("The USGS continuous source URL is unsafe.", "gx_error_usgs_continuous_plan_url")
  }
  parsed <- tryCatch(httr2::url_parse(canonical), error = function(cnd) NULL)
  path_match <- if (is.null(parsed)) integer() else regexec(
    "^/ogcapi/([A-Za-z0-9._~-]+)/collections/(continuous)/items/?$",
    parsed$path %||% "",
    perl = TRUE
  )
  ...
  if (is.null(parsed) || !identical(tolower(parsed$scheme %||% ""), "https") ||
    ...
    gx_usgs_continuous_abort(
      "M7m accepts only the reviewed USGS Water Data continuous items endpoint.",
      "gx_error_usgs_continuous_plan_url"
    )
  }
  ...
  allowed <- c(
    "monitoring_location_id", "parameter_code", "time", "properties",
    "skipGeometry", "f", "lang", "limit"
  )
  if (!length(query) || any(!query_names %in% allowed) ||
    ...
    gx_usgs_continuous_abort(
      "The inherited USGS continuous query must contain one site, one parameter code, and no unreviewed filters.",
      "gx_error_usgs_continuous_plan_url"
    )
  }
  site <- gx_usgs_continuous_bounded_text_impl(
    gx_usgs_continuous_query_value_impl(
      query, query_names, "monitoring_location_id"
    ),
    "monitoring-location identity", maximum = 128L
  )
  parameter <- gx_usgs_continuous_bounded_text_impl(
    gx_usgs_continuous_query_value_impl(query, query_names, "parameter_code"),
    "parameter code", maximum = 5L
  )
  if (!grepl("^USGS-[A-Za-z0-9]+$", site, perl = TRUE) ||
      !grepl("^[0-9]{5}$", parameter, perl = TRUE)) {
    gx_usgs_continuous_abort(
      "M7m requires one USGS monitoring-location identity and one five-digit parameter code.",
      "gx_error_usgs_continuous_plan_url"
    )
  }
  time <- gx_usgs_continuous_time_impl(distribution)
  limit <- unname(as.integer(min(
    50000L, max_rows, floor(max_fields / 11)
  )))
  if (limit < 1L) {
    gx_usgs_continuous_abort(
      "The M7m result limits do not admit one continuous observation.",
      "gx_error_usgs_continuous_plan_policy"
    )
  }
  expected_optional <- list(
    time = time$time,
    properties = paste(.gx_usgs_continuous_properties, collapse = ","),
    skipGeometry = "true",
    f = "json",
    lang = "en-US",
  ...
  for (name in names(expected_optional)) {
    inherited <- gx_usgs_continuous_query_value_impl(query, query_names, name)
    if (!is.null(inherited) && !identical(inherited, expected_optional[[name]])) {
      gx_usgs_continuous_abort(
        "An inherited USGS continuous query option does not match the bounded M7m request.",
        "gx_error_usgs_continuous_plan_url"
      )
    }
  }
  api_version <- gx_usgs_continuous_bounded_text_impl(
    path_parts[[2L]], "API version", maximum = 64L
  )
  collection <- path_parts[[3L]]
  endpoint_path <- sub("/$", "", parsed$path)
  base_path <- sub("/collections/continuous/items$", "", endpoint_path)
  parsed$query <- list()
...
gx_usgs_continuous_request_id_impl <- function(source, policy, row) {
  gx_contract_hash(
    list(
    ...
    namespace = "geoconnexr.usgs-continuous-request.v1",
    contract_version = .gx_usgs_continuous_request_plan_contract_version
  )
}

gx_usgs_continuous_request_impl <- function(row, source, policy) {
  byte_limit <- unname(as.double(min(
    row$reservation$max_encoded_bytes[[1L]],
    row$reservation$max_decoded_bytes[[1L]]
  )))
  list(
    logical_request_id = gx_usgs_continuous_request_id_impl(source, policy, row),
...
gx_usgs_continuous_plan_reasons_impl <- function(request_plan, request) {
  reasons <- unique(c(
    setdiff(
    ...
    "usgs_continuous_provider_transport_unauthorized",
    "usgs_continuous_result_unbound",
    "runtime_symbol_check_pending",
    "continuous_collection_only",
    "single_page_no_follow"
  ))
  reasons[gx_catalog_byte_order(reasons)]
}

gx_usgs_continuous_plan_metadata_impl <- function(request_plan, request) {
  list(
    host_specific = FALSE,
    replayable = FALSE,
    ...
    non_replayable_reasons = gx_usgs_continuous_plan_reasons_impl(request_plan, request)
  )
}

gx_usgs_continuous_request_plan_new_impl <- function(
  ...
  object <- structure(
    ...
    class = "gx_usgs_continuous_request_plan"
  )
  gx_usgs_continuous_request_plan_validate_impl(object)
  object
}

gx_usgs_continuous_request_plan_impl <- function(
    request_plan, distribution_id, max_fields = NULL) {
  policy <- gx_usgs_continuous_policy_impl(max_fields)
  row <- gx_usgs_continuous_source_row_impl(request_plan, distribution_id)
  source <- gx_usgs_continuous_target_impl(
    row$distribution$distribution_url[[1L]], row$distribution,
    row$max_rows, policy$max_fields
  )
  request <- gx_usgs_continuous_request_impl(row, source, policy)
  gx_usgs_continuous_request_plan_new_impl(
    request_plan = request_plan,
    policy = policy,
    request = request,
    metadata = gx_usgs_continuous_plan_metadata_impl(request_plan, request)
  )
}

gx_usgs_continuous_request_plan_validate_body <- function(x) {
  valid_top <- identical(class(x), "gx_usgs_continuous_request_plan") &&
    identical(names(x), .gx_usgs_continuous_request_plan_fields) &&
    identical(names(attributes(x)), c("names", "class")) &&
    identical(x$contract_version, .gx_usgs_continuous_request_plan_contract_version)
  if (!valid_top) {
    gx_usgs_continuous_abort("The USGS continuous request plan violates its exact top-level contract.")
  }
  row <- gx_usgs_continuous_source_row_impl(
    x$request_plan, x$request$distribution_id %||% NA_character_
  )
  if (!gx_usgs_continuous_exact_names(x$policy, .gx_usgs_continuous_policy_fields) ||
      !gx_usgs_continuous_exact_names(x$request, .gx_usgs_continuous_request_fields) ||
      !gx_usgs_continuous_exact_names(x$metadata, .gx_usgs_continuous_plan_metadata_fields)) {
    gx_usgs_continuous_abort("The USGS continuous request plan contains malformed nested facts.")
  }
  policy <- gx_usgs_continuous_policy_impl(x$policy$max_fields)
  source <- gx_usgs_continuous_target_impl(
    row$distribution$distribution_url[[1L]], row$distribution,
    row$max_rows, policy$max_fields
  )
  request <- gx_usgs_continuous_request_impl(row, source, policy)
  metadata <- gx_usgs_continuous_plan_metadata_impl(x$request_plan, request)
  if (!identical(x$policy, policy) || !identical(x$request, request) ||
      !identical(x$metadata, metadata)) {
    gx_usgs_continuous_abort(
      "The USGS continuous request plan no longer rebinds to its source and reservation.",
      "gx_error_usgs_continuous_plan_binding"
    )
  }
  invisible(x)
}

gx_usgs_continuous_request_plan_validate_impl <- function(x) {
  tryCatch(
    gx_usgs_continuous_request_plan_validate_body(x),
    error = function(cnd) {
      if (inherits(cnd, "gx_error_usgs_continuous")) stop(cnd)
      gx_usgs_continuous_abort("USGS continuous request-plan validation rejected a malformed object.")
    },
    warning = function(cnd) {
      gx_usgs_continuous_abort(
        "USGS continuous request-plan validation rejected a warning-producing object."
      )
    }
  )
}
...
#' @export
print.gx_usgs_continuous_request_plan <- function(x, ...) {
  gx_usgs_continuous_request_plan_validate_impl(x)
  cli::cli_inform(c(
    "<gx_usgs_continuous_request_plan>",
    "* Collection: {x$request$collection_id}; site: {x$request$monitoring_location_id}",
    "* Parameter: {x$request$parameter_code}; page limit: {x$request$limit}",
    "* Reserved attempts: 1; requests executed: 0"
  ))
  invisible(x)
}

gx_usgs_continuous_json_text_impl <- function(body) {
  if (!is.raw(body) || !is.null(attributes(body)) || !length(body)) {
    gx_usgs_continuous_abort(
      "The USGS continuous response body must contain bounded raw GeoJSON bytes.",
      "gx_error_usgs_continuous_payload"
    )
  }
  ...
  if (!valid) {
    gx_usgs_continuous_abort(
      "The USGS continuous response is not one BOM-free UTF-8 JSON document.",
      "gx_error_usgs_continuous_payload"
    )
  }
  Encoding(text) <- "UTF-8"
...
gx_usgs_continuous_assert_unique_members_impl <- function(value) {
  stack <- list(value)
  while (length(stack)) {
    ...
    if (!is.null(object_names) &&
      ...
      gx_usgs_continuous_abort(
      "The USGS continuous GeoJSON contains duplicate or invalid object members.",
        "gx_error_usgs_continuous_payload"
      )
    }
...
gx_usgs_continuous_json_impl <- function(body, policy) {
  text <- gx_usgs_continuous_json_text_impl(body)
  preflight <- tryCatch({
  ...
  if (!preflight) {
    gx_usgs_continuous_abort(
      "The USGS continuous GeoJSON exceeds its structural parsing limits.",
      "gx_error_usgs_continuous_payload"
    )
  }
  document <- tryCatch(
  ...
  if (!is.list(document) || is.null(names(document))) {
    gx_usgs_continuous_abort(
      "The USGS continuous response is not one GeoJSON object.",
      "gx_error_usgs_continuous_payload"
    )
  }
  gx_usgs_continuous_assert_unique_members_impl(document)
  document
}

gx_usgs_continuous_json_array_impl <- function(x, label, allow_null = FALSE) {
  if (!is.list(x) || (!allow_null && any(vapply(x, is.null, logical(1))))) {
    gx_usgs_continuous_abort(
      "The USGS continuous GeoJSON {label} array is malformed.",
      "gx_error_usgs_continuous_payload"
    )
  }
  x
...
gx_usgs_continuous_json_object_impl <- function(x, label) {
  valid <- is.list(x) && !is.null(names(x)) && length(x) > 0L &&
    !anyNA(names(x)) && all(nzchar(names(x))) && !anyDuplicated(names(x))
  if (!valid) {
    gx_usgs_continuous_abort(
      "The USGS continuous GeoJSON {label} object is malformed.",
      "gx_error_usgs_continuous_payload"
    )
  }
  x
...
gx_usgs_continuous_datetime_impl <- function(value) {
  if (!gx_usgs_continuous_scalar_text(value)) return(NA_real_)
...
gx_usgs_continuous_schema_impl <- function(data) {
...
gx_usgs_continuous_whole_impl <- function(x, label, allow_null = FALSE) {
  ...
  if (!valid) {
    gx_usgs_continuous_abort(
      "The USGS continuous GeoJSON {label} is not one bounded whole number.",
      "gx_error_usgs_continuous_payload"
    )
  }
  unname(as.integer(x))
...
gx_usgs_continuous_property_text_impl <- function(
    value, label, allow_null = FALSE) {
  if (allow_null && is.null(value)) return(NA_character_)
  valid <- gx_usgs_continuous_scalar_text(value) &&
  ...
  if (!valid) {
    gx_usgs_continuous_abort(
      "The USGS continuous GeoJSON {label} is not one bounded control-safe UTF-8 string.",
      "gx_error_usgs_continuous_payload"
    )
  }
...
gx_usgs_continuous_feature_impl <- function(feature, request_plan) {
  request <- request_plan$request
  feature <- gx_usgs_continuous_json_object_impl(feature, "feature")
  if (!setequal(names(feature), c("type", "id", "geometry", "properties")) ||
      !identical(feature$type, "Feature") ||
      !gx_usgs_continuous_scalar_text(feature$id) ||
      !is.null(feature$geometry)) {
    gx_usgs_continuous_abort(
      "M7m requires identified GeoJSON Features with omitted geometry.",
      "gx_error_usgs_continuous_payload"
    )
  }
  properties <- gx_usgs_continuous_json_object_impl(
    feature$properties, "feature properties"
  )
  if (length(properties) != length(.gx_usgs_continuous_properties) ||
      !setequal(names(properties), .gx_usgs_continuous_properties)) {
    gx_usgs_continuous_abort(
      "A USGS continuous feature does not have the exact requested property set.",
      "gx_error_usgs_continuous_payload"
    )
  }
  values <- lapply(.gx_usgs_continuous_properties, function(name) {
    gx_usgs_continuous_property_text_impl(
      properties[[name]], name, allow_null = identical(name, "qualifier")
    )
  })
  names(values) <- .gx_usgs_continuous_properties
  if (!identical(values$monitoring_location_id, request$monitoring_location_id) ||
    ...
    gx_usgs_continuous_abort(
      "A USGS continuous feature does not match the planned site and parameter.",
      "gx_error_usgs_continuous_payload"
    )
  }
  observed_time <- gx_usgs_continuous_datetime_impl(values$time)
  modified_time <- gx_usgs_continuous_datetime_impl(values$last_modified)
  start <- gx_usgs_continuous_datetime_impl(request$time_start)
  end <- gx_usgs_continuous_datetime_impl(request$time_end)
  if (anyNA(c(observed_time, modified_time, start, end)) ||
      observed_time < start || observed_time > end) {
    gx_usgs_continuous_abort(
      "A USGS continuous feature has invalid or out-of-window RFC 3339 time data.",
      "gx_error_usgs_continuous_payload"
    )
  }
  list(
    continuous_id = gx_usgs_continuous_property_text_impl(
    ...
    time = as.POSIXct(observed_time, origin = "1970-01-01", tz = "UTC"),
    value = values$value,
...
gx_usgs_continuous_data_impl <- function(features, request_plan) {
  features <- gx_usgs_continuous_json_array_impl(
  ...
  if (length(features) > request_plan$request$limit ||
    ...
    gx_usgs_continuous_abort(
      "The USGS continuous page exceeds its planned row limit.",
      "gx_error_usgs_continuous_payload"
    )
  }
  rows <- lapply(features, gx_usgs_continuous_feature_impl,
                 request_plan = request_plan)
  if (!length(rows)) {
    return(tibble::tibble(
      continuous_id = character(), time_series_id = character(),
      monitoring_location_id = character(), parameter_code = character(),
      statistic_id = character(),
      time = as.POSIXct(character(), tz = "UTC"), value = character(),
  ...
  tibble::tibble(
    continuous_id = vapply(rows, `[[`, character(1), "continuous_id"),
    ...
    time = as.POSIXct(vapply(rows, function(row) as.numeric(row$time), numeric(1)),
...
gx_usgs_continuous_next_link_impl <- function(links) {
  links <- gx_usgs_continuous_json_array_impl(links, "links")
  rels <- vapply(links, function(link) {
    link <- gx_usgs_continuous_json_object_impl(link, "link")
    rel <- gx_usgs_continuous_property_text_impl(link$rel, "link relation")
    if (identical(rel, "next")) {
      gx_usgs_continuous_property_text_impl(link$href, "next-link target")
    }
    rel
  }, character(1))
  if (sum(rels == "next") > 1L) {
    gx_usgs_continuous_abort(
      "The USGS continuous page contains multiple next links.",
      "gx_error_usgs_continuous_payload"
    )
  }
  any(rels == "next")
...
gx_usgs_continuous_strict_result_impl <- function(body, request_plan) {
  document <- gx_usgs_continuous_json_impl(body, request_plan$policy)
  required <- c(
    "type", "features", "numberReturned", "numberMatched", "links"
  )
  if (!all(required %in% names(document)) ||
    ...
    gx_usgs_continuous_abort(
      "M7m requires one USGS GeoJSON FeatureCollection page.",
      "gx_error_usgs_continuous_payload"
    )
  }
  data <- gx_usgs_continuous_data_impl(document$features, request_plan)
  number_returned <- gx_usgs_continuous_whole_impl(
    document$numberReturned, "numberReturned"
  )
  number_matched <- gx_usgs_continuous_whole_impl(
  ...
  if (number_returned != nrow(data) ||
    ...
    gx_usgs_continuous_abort(
      "The USGS continuous page counts do not match its feature data.",
      "gx_error_usgs_continuous_payload"
    )
  }
  next_link <- gx_usgs_continuous_next_link_impl(document$links)
  truncated <- next_link ||
    (!is.na(number_matched) && number_matched > number_returned) ||
  ...
  if (ncol(data) > request_plan$request$max_columns ||
    ...
    gx_usgs_continuous_abort(
      "The normalized USGS continuous result exceeds its planned shape limits.",
      "gx_error_usgs_continuous_payload"
    )
  }
  schema <- gx_usgs_continuous_schema_impl(data)
  parse <- list(
    ...
    parser_validation = "native_geojson_exact_continuous_subset"
  )
  list(data = data, schema = schema, parse = parse)
}

gx_usgs_continuous_result_impl <- function(body, request_plan) {
  gx_usgs_continuous_strict_result_impl(body, request_plan)
}

gx_usgs_continuous_candidate_impl <- function(response, request_plan, target) {
  ...
  if (!valid) {
    gx_usgs_continuous_abort(
      "The provider response violated the planned USGS continuous response envelope.",
      "gx_error_usgs_continuous_response"
    )
  }
  gx_usgs_continuous_result_impl(response$body, request_plan)
}

gx_usgs_continuous_symbol_resolver_impl <- function(
  ...
  required_formals <- c(
    "monitoring_location_id", "parameter_code", "properties", "time",
    "limit", "no_paging"
  )
...
gx_handler_usgs_waterdata_continuous <- function(
    request_plan, timeout, min_interval, capability) {
  gx_usgs_continuous_request_plan_validate_impl(request_plan)
  source <- gx_usgs_continuous_source_row_impl(
    request_plan$request_plan, request_plan$request$distribution_id
  )
  target <- gx_usgs_continuous_target_impl(
  ...
  validator <- function(response) {
    result <<- gx_usgs_continuous_candidate_impl(response, request_plan, target)
    invisible(NULL)
  }
  ...
  if (is.null(result)) {
    gx_usgs_continuous_abort(
      "The USGS continuous provider response bypassed its result validator.",
      "gx_error_usgs_continuous_response"
    )
  }
  list(
...
gx_usgs_continuous_invoke_impl <- function(
    request_plan, timeout, min_interval, symbol_resolver) {
  resolver <- symbol_resolver %||% gx_usgs_continuous_symbol_resolver_impl
  if (!is.function(resolver)) {
    gx_usgs_continuous_abort(
      "The USGS continuous runtime implementation resolver is invalid.",
      "gx_error_usgs_continuous_execution_capability"
    )
  }
  capability <- tryCatch(
    resolver(
  ...
  valid <- is.list(capability) &&
    ...
    gx_usgs_continuous_scalar_text(capability$package_version) &&
  ...
  if (!valid) {
    gx_usgs_continuous_abort(
      "The required dataRetrieval version and continuous query symbol are unavailable at invocation time.",
      "gx_error_usgs_continuous_execution_capability"
    )
  }
...
gx_usgs_continuous_execution_id_impl <- function(
  ...
  gx_contract_hash(
    ...
    namespace = "geoconnexr.usgs-continuous-execution.v1",
    contract_version = .gx_usgs_continuous_execution_contract_version
  )
}

gx_usgs_continuous_attempt_id_impl <- function(execution_id, request_plan, target, attempt) {
  gx_contract_hash(
    list(
    ...
    namespace = "geoconnexr.usgs-continuous-attempt.v1",
    contract_version = .gx_usgs_continuous_execution_contract_version
  )
}

gx_usgs_continuous_attempts_impl <- function(
  ...
  if (!valid) {
    gx_usgs_continuous_abort(
      "The USGS continuous handler did not return one charged physical response attempt.",
      "gx_error_usgs_continuous_execution_attempt"
    )
  }
  ...
  attempt$attempt_id[[1L]] <- gx_usgs_continuous_attempt_id_impl(
    execution_id, request_plan, target, attempt
  )
  attempt
}

gx_usgs_continuous_implementation_impl <- function(request_plan, package_version) {
  if (!gx_usgs_continuous_scalar_text(package_version) ||
      !tryCatch(
        utils::compareVersion(
    ...
    gx_usgs_continuous_abort(
      "The recorded USGS continuous package version does not satisfy the request plan.",
      "gx_error_usgs_continuous_execution_capability"
    )
  }
  list(
...
gx_usgs_continuous_execution_facts_impl <- function(
...
gx_usgs_continuous_execution_reasons_impl <- function(request_plan) {
  reasons <- unique(c(
    setdiff(
      request_plan$metadata$non_replayable_reasons,
      c(
...
gx_usgs_continuous_execution_metadata_impl <- function(request_plan, result, bytes) {
  list(
    host_specific = TRUE,
    replayable = FALSE,
    execution_ready = FALSE,
    ...
    non_replayable_reasons = gx_usgs_continuous_execution_reasons_impl(request_plan)
  )
}

gx_usgs_continuous_redacted_attempts_impl <- function(cnd) {
...
gx_usgs_continuous_execution_impl <- function(
    request_plan,
    timeout = NULL,
    min_interval = NULL,
  ...
  execution_id <- gx_usgs_continuous_execution_id_impl(
    scope, request_plan, started_at, timeout, min_interval
  )
  invoked <- tryCatch(
    gx_usgs_continuous_invoke_impl(
      request_plan, timeout, min_interval, symbol_resolver
    ),
    error = function(cnd) {
      if (inherits(cnd, "gx_error_usgs_continuous_execution_capability")) stop(cnd)
      phase <- if (inherits(cnd, c(
        "gx_error_usgs_continuous_payload", "gx_error_csv_parse"
      ))) "parse" else "transport"
      gx_usgs_continuous_abort(
        "USGS continuous execution failed during its bounded {phase} phase; underlying details were withheld.",
        paste0("gx_error_usgs_continuous_execution_", phase),
        execution_id = execution_id,
        attempts = gx_usgs_continuous_redacted_attempts_impl(cnd)
      )
    }
  )
  completed_at <- gx_csv_execution_time_impl(gx_now())
  attempts <- gx_usgs_continuous_attempts_impl(
    invoked$response, request_plan, execution_id, invoked$target,
    invoked$result
  )
  bytes <- unname(as.double(length(invoked$response$body)))
  execution <- gx_usgs_continuous_execution_facts_impl(
    scope, execution_id, request_plan, started_at, completed_at, timeout,
  ...
  object <- structure(
    list(
      ...
      implementation = gx_usgs_continuous_implementation_impl(
  ...
  gx_usgs_continuous_execution_validate_impl(object)
  object
}

gx_usgs_continuous_execution_validate_body <- function(x) {
  valid_top <- identical(class(x), "gx_usgs_continuous_execution") &&
    identical(names(x), .gx_usgs_continuous_execution_fields) &&
    identical(names(attributes(x)), c("names", "class")) &&
    identical(x$contract_version, .gx_usgs_continuous_execution_contract_version)
  if (!valid_top) {
    gx_usgs_continuous_abort("The USGS continuous execution violates its exact top-level contract.")
  }
  gx_usgs_continuous_request_plan_validate_impl(x$request_plan)
  if (!is.raw(x$response_body) || !is.null(attributes(x$response_body)) ||
      !inherits(x$data, "tbl_df") || !inherits(x$schema, "tbl_df") ||
      !gx_usgs_continuous_exact_names(x$parse, .gx_usgs_continuous_parse_fields) ||
      !gx_usgs_continuous_exact_names(
        x$implementation, .gx_usgs_continuous_implementation_fields
      ) || !gx_usgs_continuous_exact_names(x$execution, .gx_usgs_continuous_execution_fact_fields) ||
      !gx_usgs_continuous_exact_names(x$metadata, .gx_usgs_continuous_execution_metadata_fields)) {
    gx_usgs_continuous_abort("The USGS continuous execution contains malformed nested facts.")
  }
  rebuilt <- gx_usgs_continuous_strict_result_impl(x$response_body, x$request_plan)
  if (!identical(x$data, rebuilt$data) ||
      !identical(x$schema, rebuilt$schema) ||
      !identical(x$parse, rebuilt$parse)) {
    gx_usgs_continuous_abort(
      "The USGS continuous result no longer rebinds to its retained response bytes.",
      "gx_error_usgs_continuous_execution_result"
    )
  }
  source <- gx_usgs_continuous_source_row_impl(
  ...
  valid_attempt_shape <- inherits(attempts, "tbl_df") &&
    nrow(attempts) == 1L && identical(names(attempts), .gx_usgs_continuous_attempt_columns) &&
    !anyNA(attempts) && all(vapply(attempts, function(column) {
      is.null(attributes(column))
    }, logical(1)))
  if (!valid_attempt_shape) {
    gx_usgs_continuous_abort("The USGS continuous attempt ledger violates its exact shape.")
  }
  bytes <- unname(as.double(length(x$response_body)))
  expected_implementation <- gx_usgs_continuous_implementation_impl(
  ...
  expected_execution_id <- if (!is.null(scope) && !is.null(timeout) &&
    ...
    gx_usgs_continuous_execution_id_impl(
  ...
  attempt_values <- attempts$contract_version[[1L]] ==
      .gx_usgs_continuous_execution_contract_version &&
  ...
  expected_attempt_id <- if (attempt_values) {
    gx_usgs_continuous_attempt_id_impl(
  ...
  expected_metadata <- gx_usgs_continuous_execution_metadata_impl(
  ...
  if (!identical(x$implementation, expected_implementation) ||
    ...
    gx_usgs_continuous_abort(
      "The USGS continuous execution facts no longer rebind to its plan and response.",
      "gx_error_usgs_continuous_execution_binding"
    )
  }
  owned_text <- gx_fetch_plan_text_total(
    x, limit = .gx_usgs_continuous_max_text_bytes
  )
  if (!is.finite(owned_text) || owned_text > .gx_usgs_continuous_max_text_bytes) {
    gx_usgs_continuous_abort(
      "The USGS continuous execution exceeds its aggregate text budget.",
      "gx_error_usgs_continuous_execution_budget"
    )
  }
  invisible(x)
}

gx_usgs_continuous_execution_validate_impl <- function(x) {
  tryCatch(
    gx_usgs_continuous_execution_validate_body(x),
    error = function(cnd) {
      if (inherits(cnd, "gx_error_usgs_continuous")) stop(cnd)
      gx_usgs_continuous_abort("USGS continuous execution validation rejected a malformed object.")
    },
    warning = function(cnd) {
      gx_usgs_continuous_abort(
        "USGS continuous execution validation rejected a warning-producing object."
      )
    }
  )
}
...
#' @export
print.gx_usgs_continuous_execution <- function(x, ...) {
  gx_usgs_continuous_execution_validate_impl(x)
  cli::cli_inform(c(
    "<gx_usgs_continuous_execution>",
