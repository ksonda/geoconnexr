...
and fixture tests for every non-CSV handler; runtime package/symbol
recheck coupled to invocation; multi-request/provider execution and
ledgers; poisoned redirect; missing package; no-network dry run; page
and aggregate budgets; one handler failure does not abort unrelated
handlers; exact one-to-one status reconciliation; reviewed registration
and serialization/replay contracts.
