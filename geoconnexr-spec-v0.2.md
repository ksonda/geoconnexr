...
**Status:** Candidate plan; implementation is gated on P0 decisions and the architecture spike below
**Version:** 0.2.0
**Reviewed:** 2026-07-12
...
These observations were checked against production or current primary package documentation on 2026-07-12. They are evidence for this plan, not permanent API guarantees.
...
| NLDI | `huc12pp` is advertised; a checked HUC12 returned both COMID and mainstem fields | Prefer the returned mainstem, then COMID/VAA fallback |
...
- **Imports:** R (>= 4.1), `cachem`, `cli`, `digest`, `httr2`, `jsonld`, `jsonlite`, `rlang`, `sf`, `tibble`, `vctrs`, `xml2`, `yaml`.
...
1. package name and owning GitHub organization;
2. MIT vs Apache-2.0 license;
3. CRAN intent and release channel;
4. `mainstems` vs `mainstems_v3` default and vintage/migration policy;
5. whether graph POST-at-root is a supported public contract;
6. support/ownership for weekly live-service alerts.
...
- retry 429 and selected 5xx/transport failures, honor `Retry-After`, and use jitter;
...
`gx_ref_feature()` tries the item endpoint, then a validated `id` collection filter, then negotiated JSON-LD. It returns a classed error only after all compatible paths fail.
...
- COMID→mainstem uses `nhdplusTools::get_vaa(atts = "levelpathi")`; missing `nhdplusTools` produces an actionable classed error.
- Check current/superseded state in the P0-selected mainstem vintage and expose both requested and current URIs.
- HUC12 outlet mode calls NLDI `huc12pp`, preferring its mainstem value, then its COMID/VAA mapping. Intersects mode returns every spatial match and explicit ranking metrics; it never silently selects one.
...
**Acceptance:** retain the checked gage answer, add checked HUC12 and superseded fixtures, test leading zeros/duplicates/NA/not-found inputs, and verify all identifier columns remain character.
...
Named SELECT templates declare parameter types, stable ordering, page strategy, result key, and row budget in a bundled manifest. Only named templates paginate. URI-list/VALUES parameters are typed and chunked; scalar URI, literal, integer, datetime, and WKT parameters are escaped by dedicated encoders.
...
```r
gx_aoi(x, type = c("auto", "huc", "county", "state",
                   "mainstem_basin", "sf", "point_upstream"))
...
Recipes embed normalized input identifiers or canonical GeoJSON/WKB plus CRS for custom geometries. `mainstem_basin` is implemented only after P0 confirms its delineation source; otherwise it is removed from the public signature.
...
**Acceptance:** HUC8, county, and NLDI-basin fixtures; leading-zero IDs; custom geometry replay; partial graph/reference/JSON-LD failures; deduplication and provenance; count reconciliation across data and diagnostics.
...
Frictionless compatibility is validated with the Python Frictionless CLI in CI; the R `frictionless` package alone is not treated as comprehensive validation. Profile/non-tabular resources explicitly.

**Acceptance:** catalog-only and fetched packages; offline checksum verification; replay from HUC and custom-geometry recipes; tampered file detection; missing Arrow/Quarto behavior; full Frictionless CLI validation of the chosen profile.
...
1. name/namespace and repository owner;
2. license;
3. CRAN/r-universe release policy;
4. mainstem vintage/default and migration behavior;
5. supported SPARQL public endpoint contract;
6. whether administrative layers such as water rights are future scope;
7. live-monitor ownership and response expectations.
